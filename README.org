...
** Using uv
The by far easiest way to use these tools is via [[https://docs.astral.sh/uv/][uv]], a package and project manager for Python.   With uv installed, you can simply run the tools as below and everything that’s needed will be automatically installed in a new virtual environment.
