...
Use a transformer model to generate tokens and calculate per-token surprisal.
...
Use a transformer model to generate ranking of the N most likely next tokens.
