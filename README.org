...
- Runs open source pre-trained LLM locally without the need for an API key or internet connection.  Some of the preconfigured models (check file:models.py for full list):
...
When no input text is provided, ~llm_topn.py~ interactively prompts the user for text.  Useful for quick experiments since the model has to be loaded only once.
...
#+RESULTS:
#+begin_example
Welcome to llm_topn interactive mode.
Enter text: The key to the cabinets
Item                    Text Token Rank: Surprisal (bits)
   1 The key to the cabinets    is    1: ██                 1.5
   1 The key to the cabinets   are    2: ████               4.1
   1 The key to the cabinets     ,    3: ████               4.2
...
For the full license text, consult the file file:LICENSE in this repository or visit the [[https://www.gnu.org/licenses/gpl-3.0.html][GNU website]].

* How to cite
If you use this software in academic research, please cite it using the following format.

APA:
- von der Malsburg, T. (2026). llm_surprisal: Command-line tools for low-barrier reproducible experiments with transformer models. GitHub repository. https://github.com/tmalsburg/llm_surprisal

BibTeX:
#+BEGIN_SRC bibtex
@Misc{Malsburg2026llmsurprisal,
  author    = {von der Malsburg, Titus},
  title     = {llm\_surprisal: Command-line tools for low-barrier reproducible experiments with transformer models},
  howpublished= {\url{https://github.com/tmalsburg/llm_surprisal}},
  year      = {2026},
  publisher = {GitHub},
