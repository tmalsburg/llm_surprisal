...
To obtain in ~.csv~ format which can be easily loaded in R, Excel, Google Sheets, and similar:
#+BEGIN_SRC sh :eval no
python3 gpt2_generate.py "The key to the cabinets" -n 4 -o output.csv
#+END_SRC

When no output file is specified (default output in the shell), CSV format can be forced with the ~-c~ option:
